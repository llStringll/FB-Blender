  ...
  def __call__(self, texts):
    ...
    for text,b in texts[::-1][:self.max_history]:
      ...
      if "your persona:" in text:
          segment_ids = [1] * len(input_ids) # segment id for presona
      elif b==False:
          segment_ids = [1] * len(input_ids) # segment id for speaker speech
      else:
          segment_ids = [0] * len(input_ids) # segment if for past bot speech, consistent with segment id assigned to responses
